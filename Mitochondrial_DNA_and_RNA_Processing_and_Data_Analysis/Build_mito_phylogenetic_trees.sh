...
## join the tree data with metadata from above
x <- left_join(as_tibble(tree), tree_labels, by = c("label" = "name"))  %>% mutate(name=label)


## make tree including clade labels.  Will have to change clade labels for data set.  Will need to find the nodes for each set of samples by uncommenting #geom_label(aes(label=node))
